...
function drunkPower(name){
    var grabName = name.first_name; //grab first names
    var toString = grabName.toString(); // change to string
   var convert = toString.toUpperCase(); // convert to Uppercase
...
allCaps = runners.map(drunkPower); // mapping function to object
...
let largeShirts = [];
largeShirts = runners.filter(shirtCheck);

function shirtCheck(x){
...
function pushTo(x){
 ...
 ticketPriceTotal.push(x.donation);};
